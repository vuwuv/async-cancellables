...
class CancellationToken extends EventEmitter {
    ...
    #attachTo(parents) {
        ...
        for (let parent of parents) {
            ...
            if (!isSet && this.#parents && this.#parents !== parent) {
                this.#parents = new Set([this.#parents]);
                isSet = true;
            }

            if (isSet ? !this.#parents.has(parent) : this.#parents !== parent) {
                if (!this.#cancelled) {
                    if (parent.cancelled) this.#cancel(parent.cancelledBy);
                    else parent.once('cancel', this.#cancel.bind(this), this);
                }
                isSet ? this.#parents.add(parent) : (this.#parents = parent);
            }
        }
    }
